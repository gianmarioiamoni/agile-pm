...
/**
 * Page for assigning tasks to a sprint
 * @param {object} props - React props
 * @param {object} props.params - Route params
 * @property {string} params.sprintId - ID of the sprint
 */
export default function SprintTasksAssignmentPage() {
    // // const { sprintId } = props.params;
    // const sprintId = useParams();
    // console.log("SprintTasksAssignmentPage: sprintId:", sprintId);
    // const [availableTasks, setAvailableTasks] = useState([]);
    // const [sprintTasks, setSprintTasks] = useState([]);
    // const [sprintName, setSprintName] = useState('');

    // /**
    //  * Fetch available tasks and sprint tasks when the component mounts
    //  */
    // useEffect(() => {
    //     const fetchAvailableTasks = async () => {
    //         console.log("SprintTasksAssignmentPage: fetchAvailableTasks: start");
    //         const tasks = await getAvailableTasksAndSprintTasks();
    //         console.log("SprintTasksAssignmentPage: fetchAvailableTasks: tasks:", tasks);
    //         setAvailableTasks(tasks.tasks);
    //         const sprintTasks = tasks.sprints.find((sprint) => sprint._id === sprintId);
    //         console.log("SprintTasksAssignmentPage: fetchAvailableTasks: sprintTasks:", sprintTasks);
    //         setSprintTasks(sprintTasks.tasks);
    //         setSprintName(tasks.sprints.find((sprint) => sprint._id === sprintId).name);
    //         console.log("SprintTasksAssignmentPage: fetchAvailableTasks: end");
    //     };
    //     fetchAvailableTasks();
    // }, []);
    ...
    //  * @param {object} result - Drag end result
    //  */
    // const handleDragEnd = async (result) => {
    //     const { source, destination } = result;

    //     // If the user dropped the task outside the list
    //     if (!destination) {
    //         console.log("Destination is undefined");
    //         return;
    //     }

    //     // If the task was moved to the same list
    //     if (source.droppableId === destination.droppableId) {
    //         const items = source.droppableId === 'availableTasks' ? Array.from(availableTasks) : Array.from(sprintTasks);
    //         const [removed] = items.splice(source.index, 1);
    //         items.splice(destination.index, 0, removed);

    //         // Update the state and save the changes to the database
    ...
    // /**
    //  * Get the color of the task based on its status
